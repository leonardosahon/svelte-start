...
{#snippet toolbarBtn()}
	<Popover.Root open={popoverOpened} onOpenChange={(e) => (popoverOpened = !popoverOpened)}>
		...
		<Popover.Content>
			...
			<Separator class="border-accent my-2" />
...
<Modal title="Pick an Image" bind:open={modalOpened} centered variant="xl">
	...
	{#if serverImages.length > 0}
		<div class="bg-background sticky top-0 z-1 flex gap-2 pb-3">
			<InputGroup.Root>
				<InputGroup.Input
		...
		<div class="flex justify-center">
			{#if uploadData.file?.length}
				<div class="border-r-input mr-5 flex-4 border-r-2 pr-5 md:basis-9/10">
					{@render send2Server()}
				</div>
			{/if}
			...
			<!-- Big Preview -->
			{#if selectedImage}
				<div class="border-l-input ml-5 hidden flex-4 border-l-2 pl-5 md:block md:basis-9/10">
